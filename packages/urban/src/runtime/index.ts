...
// The GROUP-BY sibling `defineRollup` (ADR 0065, #468): a declare-once aggregate rollup compiled to
// BOTH a managed `*_counts` VIEW and a TS group-reduce (closed aggregate op set: `count`/`countWhere`/
// `max`/`minWhere`/`coalesce`/`add`), registered as a canonical projection via `rollupRegistry` and
// parity-guarded by `assertRollupParity`. `defineReadModel` gains a key-correlated rollup lookup
// (`ReadModelDecl.lookups` + `rcol`) so per-row derivations read a rollup's columns for the base row.
export {
  add,
  and,
