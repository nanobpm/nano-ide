...
export function emitDomainDts(tables: TableMeta[]): string {
  ...
  if (tables.length === 0) {
    return `${header}\nexport type DomainTables = Record<never, never>;\n`;
  }
...
export function emitDomainDtsForSources(
  ...
  const sourceMap = sources
    .map((s) => {
      if (s.tables.length === 0) return `  ${JSON.stringify(s.source)}: Record<never, never>;`;
...
export function emitDomainBindings(
  ...
  return `${header}\n` +
    `import { type DataSource, openDataSource, type Table } from "./data-sdk.ts";\n` +
    importTypes +
    `\n${domainIface}\n\n${openFn}\n\nexport type { Table };\n`;
}
...
function emitKeyedDomainBindings(
  ...
  const openFn =
    ...
    `  // biome-ignore lint/plugin: the per-source table set is bound dynamically from DOMAIN_TABLES; the Domain<M> shape can't be expressed statically over the runtime loop.\n` +
