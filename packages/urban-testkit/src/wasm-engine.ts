...
export class WasmEngineClient implements EngineClient {
  ...
  async deployResources(
    ...
    //
    // The authored XML is deployed UNMODIFIED: a `<bpmn:callActivity>` reaches the engine as-is and
    // executes natively — the engine instantiates the called process as a real child instance and
    // the parent parks until it completes (`@nanobpm/engine-wasm` ≥ 0.6.0). Consequence for
    // consumers: the called process must be deployed to this same engine, or the engine raises a
    // recoverable `CalledElementError` incident on the call activity — production-faithful behaviour.
    for (const r of resources) {
      if (isEngineModel(r)) this.#liveEngine.deploy(r.content);
    }
  ...
  async drain(): Promise<void> {
    for (let i = 0; i < MAX_DRAIN_ITERATIONS; i++) {
      ...
      for (const jobType of this.#dispatchableJobTypes()) {
        ...
        // A mock-only type has no registered worker; use a synthetic activation
  ...
  /** The job types to activate on a drain pass: every registered worker, plus every mock-only type
   *  that actually carries at least one clause (an empty/reset mock induces no dispatch).
   *  De-duplicated so a type that lands in more than one source is only activated once per pass. */
