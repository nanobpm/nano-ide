...
describe("app e2e (urban-testkit)", () => {
	...
	before(async () => {
		...
		assert.ok(
			app.api,
			"app.api should be defined (nano.app.json declares an `api` binding)",
		);
	});
	...
	test("records a greeting through the full POST → process → worker → GET pipeline", async () => {
		...
		const created = await api.call<{ accepted: boolean; who: string }>(
			"createGreeting",
			{
				body: { who: "Ada" },
			},
		...
		assert.ok(
			app.coverage,
			"coverage gate is enabled (bootTestApp was called with { coverage: true })",
		);
