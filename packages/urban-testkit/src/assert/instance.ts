...
// `TestApp`-based public API by adapting the app onto that port via
// `engineReadModel(app)` — which delegates `snapshot()` to the overridable
// `app.snapshot()` seam (not `app.engine.snapshot()`) — and forwarding it to the
// lifted matcher. The failure-message behaviour, selectors and incident handling
// are unchanged; they are simply sourced from their single canonical home.
